...
| **1 ✅ done 2026-09-10** | Micrometer + Prometheus-client backends both built behind the `MetricsBackend` SPI (Spike A → Micrometer default), `/metrics` served only when enabled, `ActionTimingPhaseListener` + `ActionNameResolver` (redirect-aware, poll filter), OTel Collector run-scoped with `run_id`/`release`/`test_type`/`env`, Dashboard 1 provisioned. Findings in `docs/findings/`. | A, B, C | met: toggle off→404 / on→timers; runs 0.1.0/0.2.0/0.3.0 in Prometheus tagged by run; poll requests excluded; dashboard renders p95 by action (`login` p95 ≈ 0.133 s vs 120 ms simulated) |
| **2 ✅ done 2026-09-10** | k6-browser driver (records `client_action_seconds`, remote-writes with run tags); recording rules (`run:portal_user_action_seconds:p50/p95/p99`, rate, error_ratio); Dashboard 2 "Baseline vs Candidate" (bar + delta table + `offset` overlay + client-vs-server) and Dashboard 3 "Release Trend"; `ci/Jenkinsfile` + `.github/workflows/build.yml`. Findings D, F. | D, F | met: Selenium `rel-1.0.0` + k6-browser `rel-1.1.0` runs both produce the same server-side actions; bar/delta/trend queries verified; dashboards provision clean; 5 runs available for the trend |
| **3 ✅ done 2026-09-10** | `metrics-jaxrs` timing filter + `metrics-servlet` (shared `/metrics`); `service-a` (patient directory) + `service-b` (codes + validate) JAX-RS WARs emitting `service_endpoint_seconds{service,route,method,status,outcome}`; `k6-http` service driver; "Service Endpoints — Load Test" dashboard + recording rules | — | met: 6-VU/90s run → 985+ req/endpoint in Prometheus, server-side p95 per endpoint in Grafana, route templates (`/patients/{ref}`) keep cardinality bounded |
| **4 ✅ done 2026-09-10** | App-server/JVM metrics via WildFly/EAP **MicroProfile Metrics subsystem** (`:9990/metrics`) — no agent, no app Java (Spike E); "App Server & JVM — Load Test" dashboard; `scripts/on-vm-jmx-exporter.sh` for the standalone-jmx_exporter alternative | E | met: collector scrapes `:9990`, 232 `wildfly` series/run with run labels, dashboard renders heap/GC/threads/CPU/Undertow. Porting notes (Spike G) deferred at the engineer's request. |
